document.addEventListener 'DOMContentLoaded', ->
  ...
  previewDocuments = document.getElementById 'preview-documents'
  wordCount = document.getElementById 'word-count'
  progressCircle = document.getElementById 'progress-circle'
  maxWords = 280

  # Word Count and Progress Circle
  updateWordCount = ->
    text = input.value
    words = text.trim().split(/\s+/).length
    remaining = maxWords - words
    percentage = (words / maxWords) * 100
    circumference = 62.83185307179586 # 2 * π * 10
    dashOffset = circumference * (1 - (percentage / 100))
    progressCircle.style.stroke = if remaining > 50 then 'gray' else if remaining > 0 then '#f59e0b' else '#dc2626'
    progressCircle.style.strokeDashoffset = dashOffset
    wordCount.querySelector('text').textContent = remaining
    if remaining <= 0
      input.value = text.split(/\s+/).slice(0, maxWords).join(' ')
      updateWordCount()
  ...
  documentInput.addEventListener 'change', (e) ->
    ...
    previewDocuments.classList.remove 'hidden'
    for file in files
      div = document.createElement 'div'
      div.className = 'bg-gray-700 p-2 rounded-lg'
      div.innerHTML = "<p class='text-gray-300'>#{file.name}</p>"
      previewDocuments.appendChild div

  # Submit Post
  submitBtn.addEventListener 'click', ->
    text = input.value.trim()
    if text or previewPhotos.children.length or previewVideos.children.length or previewDocuments.children.length
      ...
      if previewVideos.children.length
        ...
        content += '</div>'
      if previewDocuments.children.length
        content += '<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">'
        for doc in previewDocuments.children
          content += "<div class='bg-gray-700 p-2 rounded-lg'>#{doc.innerHTML}</div>"
        content += '</div>'
      content += """
      ...
      previewDocuments.classList.add 'hidden'
